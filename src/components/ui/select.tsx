    ...
    <SelectPrimitive.Portal container={portalContainer}>
      <SelectPrimitive.Positioner align="start" alignItemWithTrigger={false} className="z-100" sideOffset={6}>
        <SelectPrimitive.Popup
          className={`max-h-64 w-[var(--anchor-width)] max-w-[calc(100vw-2rem)] min-w-36 overflow-y-auto rounded-xl border border-line-popover bg-surface-card p-1 text-sm text-ink shadow-[var(--shadow-popover)] outline-none ${className ?? ''}`}
        >
