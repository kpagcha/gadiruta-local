...
│   ├── data/                     Browser-side network loading, validation, and search logic
...
CTAN GTFS ZIP
  ...
  → scripts/build-network-data.ts selects Bahía routes, stops, and patterns
...
JSON diff before committing. The app can be developed from a fresh checkout without the ZIP because
